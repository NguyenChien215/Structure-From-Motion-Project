...
from Config import MY_DATA_IMG1, MY_DATA_IMG2, DAVID_LOWE_COEFFICIENT, my_K
...
# Load the images
img1 = cv2.imread(MY_DATA_IMG1, cv2.IMREAD_GRAYSCALE)
img2 = cv2.imread(MY_DATA_IMG2, cv2.IMREAD_GRAYSCALE)
...
# Save the images
cv2.imwrite("output/custom_image/SIFT_Detector_Image1.png", img1_store)
cv2.imwrite("output/custom_image/SIFT_Detector_Image2.png", img2_store)
...
# Store images
cv2.imwrite("output/custom_image/Matching_TwoImages_OnlyBFM_AllMatches.png", bf_result)

# Draw matches again with 1000 matches
bf_result = cv2.drawMatches(img1, kp1, img2, kp2, bf_matches[:1000], None, **bf_draw_params)
cv2.imwrite(f"output/custom_image/Matching_TwoImages_OnlyBFM_First1000Matches.png", bf_result)
...
cv2.imwrite(f"output/custom_image/Matching_TwoImages_AfterLowRatio_{len(good_matches)}Matches.png", bf_result)
...
cv2.imwrite(f"output/custom_image/Matching_TwoImages_HomoRANSAC_{len(inlier_matches)}Matches.png", homo_result)
...
if len(inlier_matches) > 8: 
    ...
    with open("output/custom_image/Fundamental_matrix.txt", "w") as f:
    ...
    cv2.imwrite("output/custom_image/Combined_kp1_epipolar2.png", combined)
    combined = np.hstack((kp_epipolar2, epipolar1))
    cv2.imwrite("output/custom_image/Combined_kp2_epipolar1.png", combined)
...
E = my_K.T @ F @ my_K
with open("output/custom_image/Essential_matrix.txt", "w") as f:
...
for i, (R, T) in enumerate(possible_poses):
    ...
    # This removes lens distortion effects using the intrinsic matrix (my_K)
    pts1_h = cv2.undistortPoints(inlier_pts1.reshape(-1, 1, 2), my_K, None)
    pts2_h = cv2.undistortPoints(inlier_pts2.reshape(-1, 1, 2), my_K, None)
...
# Calculated Essential Matrix
with open("output/custom_image/Essential_matrix.txt", "a") as f:
    f.write("\nRotation Matrix (R):\n")
...
# Store the 3D points
np.savetxt("output/custom_image/3D_points.txt", best_points.T, fmt="%.6f")
...
# Save image
plt.savefig("output/custom_image/3D_reconstruction.png", dpi=300)
plt.show()
